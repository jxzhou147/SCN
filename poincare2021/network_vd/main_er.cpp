...
*/
        /*
        if ((t > 150) & (fmod(t, daylength) >= (daylength / 2.0)))
        {
            light = 1;
        }
        else
...
*/
    ...
    numEntrain++;
    if ((abs(P[0] - daylength) < 0.1) & (abs(P[1] - daylength) < 0.1) & (abs(P[2] - daylength) < 0.1))
        ifEntrain++;
    pthread_rwlock_unlock(&rwlock);

    //if (numEntrain == 30)
    //{
    write_er(daylength, P, ifEntrain);
    //}
...
*/
    ...
    for (size_t i = 0; i < nthread; i++)
    {
        iv = i / same_num;
        if (iv < 10)
            values[i] = 20.0 + 0.2 * iv;
        else
            values[i] = 26.4 + 0.2 * iv;
        pthread_create(&(threads[i]), NULL, threadfunc, values + i);
    }
