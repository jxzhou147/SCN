...
void write_results(const double t)
{
	...
	ofile << t << '\t' << N << '\t' << connection_number() << '\t' << t_re << '\t' << d << '\t' << p_link << '\t' << p_re << '\t' << Omega << '\t' << daylength << '\t' << K_f << '\t' << R[0] << '\t' << R[1] << '\t' << R[2] << '\t' << delta_theta() << '\t' << P[0] << '\t' << P[1] << '\t' << P[2] << endl;
...
int main()
{
	...
	double t = 0.0, t0 = 1300.0, t1 = 600, t2 = 900, t3 = 1200; // t0 is the time of light off
	double h = 0.1;
	double y_err[N];
