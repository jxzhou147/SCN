...
void find_peak_ave(const double t, double peak_ave[], double &peak1_ave, double &peak2_ave, const double theta_ave, int &ind_ave, const int peak_ave_num)
{
    peak1_ave = theta_ave;

    if (((peak2_ave - peak1_ave) > 5) & (ind_ave < peak_ave_num))
    {
        peak_ave[ind_ave] = t;
        ind_ave++;
    }
...
void write_results(const double t, const double (*a)[N], const double R[], const double (*peak)[5], const double peak_ave[], const double peak_ave_v[], const double peak_ave_d[], const double on_sun[], const int peak_ave_num, const double P[], const double daylength, const double tau, const double t_re, double purt, double p_link, double p_link_v, double p_link_d, double p_re, double d, double sigma, double K, double K_f, int flag)
{
    int ifEntrainment = 0;
    if (ifEntrain > 24)
        ifEntrainment = 1;
    ...
    ofstream ofile;
    ofile.open("results_peak.csv", ios::app);
    ...
    //ofile << t << '\t' << d << '\t' << p_link_v << '\t' << p_re << '\t' << K << '\t' << K_f << '\t' << R[0] << '\t' << R[1] << '\t' << R[2] << endl;
    //ofile << sigma;
    //for (size_t i = 0; i < N; i++)
...
void *threadfunc(void *arg)
{
    ...
    int peak_ave_num = 200;
    double peak_ave[peak_ave_num];
    int ind_ave = 0;
    double peak1_ave, peak2_ave;

    double peak_ave_v[peak_ave_num];
    int ind_ave_v = 0;
    double peak1_ave_v, peak2_ave_v;

    double peak_ave_d[peak_ave_num];
    int ind_ave_d = 0;
    double peak1_ave_d, peak2_ave_d;

    double on_sun[peak_ave_num];
    int ind_sun = 0;
    int on_sun1, on_sun2;
    ...
    double t = 0.0, t0 = 2022.5, t01 = 1000;
...
*/
        ...
        if (t > (t01 - 200))
        {
            find_peak_ave(t, peak_ave, peak1_ave, peak2_ave, peak_theta(theta, 0), ind_ave, peak_ave_num);
            find_peak_ave(t, peak_ave_v, peak1_ave_v, peak2_ave_v, peak_theta(theta, 1), ind_ave_v, peak_ave_num);
            find_peak_ave(t, peak_ave_d, peak1_ave_d, peak2_ave_d, peak_theta(theta, 2), ind_ave_d, peak_ave_num);

            if (ind_sun < peak_ave_num)
            {
                on_sun1 = write_light;
                if (on_sun1 < on_sun2)
                {
...
*/
    ...
    write_results(t, a, R, peak, peak_ave, peak_ave_v, peak_ave_d, on_sun, peak_ave_num, P, daylength, tau, t_re, purt, p_link, p_link_v, p_link_d, p_re, d, sigma, K, K_f, 0);
    pthread_rwlock_unlock(&rwlock);
