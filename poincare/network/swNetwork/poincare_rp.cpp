...
void random_wiring_random(int i, double (*a)[N], double d, int Nh, int Ne)
{
    //srand((unsigned)time(NULL));
...
void random_wiring_rp(int i, double (*a)[N], double d, int Nh, int Ne)
{
    //srand((unsigned)time(NULL));
    int wiring = 1;
    int wiring_rand;
    ...
    {
        wiring_rand = (rand() % N);
        if ((a[i][wiring_rand] == 0) & (dist(i, wiring_rand) > d))
            wiring = 0;
    }
...
void construct_connection_vip(double (*a)[N], double p_link, double d, double p_d, double p_vd)
{
    //srand((unsigned)time(NULL));
    for (size_t i = 0; i < N; i++)
        for (size_t j = 0; j < N; j++)
    ...
    for (size_t i = 0; i < N_v; i++)
    {
        for (size_t j = 0; j < N_v; j++)
        {
            if ((dist(i, j) <= d) & (i != j) & ((rand() % (N_rand + 1) / (float)(N_rand + 1)) < p_d))
            {
                ...
                {
                    a[i][j] = 0;
                    random_wiring(i, a, d, 0, N_v);
                }
            }
        }
        for (size_t j = N_v; j < N; j++)
        {
            if ((rand() % (N_rand + 1) / (float)(N_rand + 1)) < p_vd)
                a[i][j] = 1;
        }
    }
}
...
void construct_connection_random(double (*a)[N], double p_link_random)
{
    //srand((unsigned)time(NULL));
    ...
    for (size_t i = 0; i < N; i++)
    {
        ...
        {
            a[i][j] = 0;
            if ((rand() % (N_rand + 1) / (float)(N_rand + 1)) < p_link_random)
            {
                a[i][j] = 1;
            }
        }
...
void rewiring_random(const double y[], double (*a)[N], double tau, double p_re, double d)
{
    for (size_t i = 0; i < N; i++)
    {
        for (size_t j = 0; j < N; j++)
        {
            ...
            {
                a[i][j] = 0;
                random_wiring_random(i, a, d, 0, N);
            }
        }
    }
}
...
int kuramoto(double t, const double y[], double f[], void *params)
{
    ...
    if (t > param->t0)
        delay = 0;
    if (fmod(t + delay, 24) < (param->daylength))
        param->light = 1;
    else
        param->light = 0;
    ...
    for (size_t i = 0; i < N; i++)
    {
        ...
        if (param->e[i] != 0)
        {
            for (size_t j = 0; j < N; j++)
            {
                ...
                //if (param->light == 1)
                //  flu = 1.5;
                param->interaction[i] += param->a[j][i] * flu * y[j] / param->e[i];
...
void write_results(const double t, const double (*a)[N], const double R[], const double (*peak)[5], const double peak_ave[], const double peak_ave_v[], const double peak_ave_d[], const double on_sun[], const double P[], const double daylength, const double tau, const double t_re, double purt, double p_link, double p_link_v, double p_link_d, double p_link_random, double r_vd, double p_re, double d, double p_d, double p_vd, double sigma, double K, double K_f, int flag)
{
    int ifEntrainment = 0;
    if (ifEntrain > 24)
        ifEntrainment = 1;
    ...
    ofstream ofile;
    ofile.open("results_rp.csv", ios::app);
    ...
    //ofile << t_re << '\t' << d << '\t' << p_link_v << '\t' << p_link_d << '\t' << r_vd << '\t' << p_re << '\t' << K << '\t' << K_f << '\t' << R[0] << '\t' << R[1] << '\t' << R[2] << endl;
    ofile << d << '\t' << p_d << '\t' << p_link << '\t' << p_vd << '\t' << K << '\t' << K_f << '\t' << p_re << '\t' << daylength << '\t' << sigma << '\t' << R[0] << '\t' << R[1] << '\t' << R[2] << '\t' << P[0] << '\t' << P[1] << '\t' << P[2] << endl;
    //ofile << sigma;
    //for (size_t i = 0; i < N; i++)
...
void *threadfunc(void *arg)
{
    ...
    double sigma = 0.05;
    //double K = 0.1;
    double daylength = 12; //length of day
    double d = 1;
    //double d = 1000; // full-connected
    //double p_d = 0.6;
    double p_d = 1;
    double p_link = 0.2;
    double p_vd = 0.02;
    ...
    double purt = 0;
    double tau = 3.0;
    double a[N][N]; // a[i][j] means i->j
    double peak[N][5];
    int ind[N];
    double r_vd = 1.0;
    double p_link_vd = 0.002;
    double p_link_v = p_link_vd * r_vd / (r_vd + 1.0);
    double p_link_d = p_link_vd / (r_vd + 1.0);
    //double p_link_d = 0.01;

    double p_link_random = 0.005;
    ...
    construct_connection_vip(a, p_link, d, p_d, p_vd);
    //construct_connection(a, p_link, d, p_d);
    //construct_connection_new(a, p_link_v, p_link_d, d);
    //construct_connection_random(a, p_link_random);
    /*
    ////static network with vip neurons having half input
    for (size_t i = 0; i < N_v; i++)
    {
        for (size_t j = 0; j < N; j++)
        {
            if ((a[j][i] == 1) & ((rand() % (N_rand + 1) / (float)(N_rand + 1)) < 0.5))
            {
                a[j][i] = 0;
            }
        }
    ...
    while (t < t01)
    {
        ...
        */
        ...
        if (t1 != t2)
        {
            ...
            rewiring_random(y, a, tau, p_re, d);
...
*/
        ...
        */
        ...
        if (t > t0)
            delay = 0;
        if (fmod(t + delay, 24.0) < (daylength))
            write_light = 1;
        else
            write_light = 0;
    ...
    write_results(t, a, R, peak, peak_ave, peak_ave_v, peak_ave_d, on_sun, P, daylength, tau, t_re, purt, p_link, p_link_v, p_link_d, p_link_random, r_vd, p_re, d, p_d, p_vd, sigma, K, K_f, 0);
    pthread_rwlock_unlock(&rwlock);
    ...
    if (numEntrain == 30)
    {
        write_results(t, a, R, peak, P, daylength, tau, t_re, purt, p_link, p_re, sigma, K, K_f, 0);
    }
*/
    //write_peaks(peak);
    //write_connect(a);
