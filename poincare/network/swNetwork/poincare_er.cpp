...
*/
    ...
    for (size_t i = N_v; i < N; i++)
    {
        f[i] = gam * y[i] * (a - sqrt(pow(y[i], 2) + pow(y[i + N], 2))) - y[i + N] * omega_d * param->eta[i] + param->K * param->interaction[i] + param->K_f * param->light;
...
void write_results(const double t, const double (*a)[N], const double R[], const double (*peak)[5], const double peak_ave[], const double peak_ave_v[], const double peak_ave_d[], const double on_sun[], const double P[], const double daylength, const double tau, const double t_re, double purt, double p_link, double p_link_v, double p_link_d, double p_link_random, double r_vd, double p_re, double d, double p_d, double sigma, double K, double K_f, int flag)
{
    int ifEntrainment = 0;
    if (ifEntrain > 24)
        ifEntrainment = 1;
    ...
    ofile << K << '\t' << daylength << '\t' << P[0] << '\t' << P[1] << '\t' << P[2] << '\t' << ifEntrain << '\t' << ifEntrainment << endl;
    //ofile << d << '\t' << p_d << '\t' << p_link << '\t' << K << '\t' << K_f << '\t' << p_re << '\t' << t_re << '\t' << sigma << '\t' << R[0] << '\t' << R[1] << '\t' << R[2] << '\t' << P[0] << '\t' << P[1] << '\t' << P[2] << endl;
    //ofile << sigma;
    //for (size_t i = 0; i < N; i++)
...
*/
        ...
        */
        ...
        if (t > t0)
            delay = 0;
        if (fmod(t + delay, daylength) < (daylength / 2.0))
            write_light = 1;
        else
            write_light = 0;
    ...
    write_results(t, a, R, peak, peak_ave, peak_ave_v, peak_ave_d, on_sun, P, daylength, tau, t_re, purt, p_link, p_link_v, p_link_d, p_link_random, r_vd, p_re, d, p_d, sigma, K, K_f, 0);
    pthread_rwlock_unlock(&rwlock);
*/
    //if (p_re == 0.01)
    //  write_connect(a);
