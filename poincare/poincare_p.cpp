...
void construct_connection(double (*a)[N], double p_link)
{
    //srand((unsigned)time(NULL));
...
void find_peak_ave(const double t, double peak_ave[], double &peak1_ave, double &peak2_ave, const double theta_ave, int &ind_ave)
{
    ...
    if (((peak2_ave - peak1_ave) > 5) & (ind_ave < 20))
    {
        peak_ave[ind_ave] = t;
        ind_ave++;
    }
...
void write_results(const double t, const double (*a)[N], const double R[], const double (*peak)[5], const double peak_ave[], const double peak_ave_v[], const double peak_ave_d[], const double P[], const double daylength, const double tau, const double t_re, double purt, double p_link, double p_re, double sigma, double K, double K_f, int flag)
{
    int ifEntrainment = 0;
    if (ifEntrain > 24)
        ifEntrainment = 1;
    ...
    //ofile << sigma << '\t' << R[0] << '\t' << R[1] << '\t' << R[2] << '\t' << P[0] << '\t' << P[1] << '\t' << P[2] << endl;
    //ofile << sigma;
    //for (size_t i = 0; i < N; i++)
    //  ofile << '\t' << ((peak[i][4] - peak[i][0]) / 4);
    //ofile << endl;
    for (size_t i = 0; i < 20; i++)
        ofile << peak_ave_v[i] << '\t' << peak_ave_d[i] << '\t' << peak_ave[i] << endl;
    ofile.close();
}

void *threadfunc(void *arg)
{
    double purt = *(double *)arg;
    double p_re = 0.01;
    double K_f = 0.5;
    int tid = tnum++;
    double t_re = 0.1;
    ...
    double peak_ave_v[20];
    int ind_ave_v = 0;
    double peak1_ave_v, peak2_ave_v;

    double peak_ave_d[20];
    int ind_ave_d = 0;
    ...
    double t = 0.0, t0 = 2000.0, t01 = 3000;
...
*/
    ...
    //pthread_rwlock_wrlock(&rwlock);
    //write_results(t, a, R, peak, peak_ave, peak_ave_v, peak_ave_d, P, daylength, tau, t_re, purt, p_link, p_re, sigma, K, K_f, 0);
    //pthread_rwlock_unlock(&rwlock);
    /*
    pthread_rwlock_wrlock(&rwlock);
    numEntrain++;
...
int main(int argc, char *argv[])
{
    ...
    {
        values[i] = floor((double)i / (double)same_num) * 0.05;
